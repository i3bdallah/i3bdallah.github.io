(function () {
    ...
    const ICONS = {
        ...
        course: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>`,
        slides: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>`,
        post: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>`,
        link: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>`,
        cert: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="6"/><path d="M15.477 12.89 17 22l-5-3-5 3 1.523-9.11"/></svg>`,
    ...
    function renderHero(profile) {
        ...
        <section class="hero" data-reveal>
            <img class="hero__avatar" src="${esc(profile.avatar)}" alt="${esc(profile.name)}"
                 onerror="this.style.display='none'" />
            <h1 class="hero__name">${esc(profile.name)}</h1>
            <span class="hero__tagline">${esc(profile.tagline)}</span>
            <p class="hero__bio">${esc(profile.bio)}</p>
        </section>`;
    }

    function renderSocial(social) {
        const pills = social.map(s => {
            const target = s.url.startsWith('mailto:') ? '_self' : '_blank';
            return `<a class="social__pill" href="${esc(s.url)}" target="${target}" rel="noopener noreferrer" aria-label="${esc(s.label)}">
                ${icon(s.platform)}<span>${esc(s.label)}</span>
            </a>`;
        }).join('');
    ...
    function renderCerts(certs, startNum) {
        ...
        const badges = certs.map(c => {
            ...
            const img = c.image
                ? `<img class="cert__logo" src="${esc(c.image)}" alt="${esc(c.name)}" loading="lazy" />`
                : `<span class="cert__icon">${icon('cert')}</span>`;
            return `<${tag} class="cert"${href}>
                ${img}
                <span class="cert__info">
                    <span class="cert__name">${esc(c.name)}</span>
                    <span class="cert__org">${esc(c.org)} · ${c.year}</span>
                </span>
            </${tag}>`;
        }).join('');
        return `<section class="section" data-reveal>
